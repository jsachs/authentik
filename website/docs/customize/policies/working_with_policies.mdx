...
For an overview of policies, refer to our documentation on [Policies](./index.mdx).
...
To learn more, see [Bindings in authentik](../../add-secure-apps/bindings-overview/index.mdx) and [bind a policy to a new application when the application is created](../../add-secure-apps/applications/manage_apps.mdx#create-an-application-and-provider-pair).
...
- [Flow](../../add-secure-apps/flows-stages/flow/index.mdx)
- [Stage binding](../../add-secure-apps/flows-stages/stages/index.mdx#stage-bindings)
- [Application](../../add-secure-apps/applications/index.mdx)
- [Source](../../users-sources/sources/index.mdx)

For background about policy bindings, stage bindings, and why policies on stages are bound to flow-stage bindings, see [Bindings in authentik](../../add-secure-apps/bindings-overview/index.mdx).

When configuring a policy binding, leave **Enabled** on and turn on **Dry-run** to evaluate the policy without including its result or messages in the target's final policy decision. Save the binding and check the **Dry-run** column to confirm its mode. Click **View dry-run results** in the binding's actions to open the filtered Event Log. Turn **Dry-run** off to enforce the binding again. See [Dry-run](./bindings.mdx#dry-run) for detailed evaluation behavior.
...
If the policy depends on request data that is only known after the user has interacted with the flow, configure the stage binding to **Evaluate when stage is run**. For more information, see [Planning and stage policies](../../add-secure-apps/flows-stages/flow/planner.mdx#planning-and-stage-policies).
...
For background on policy ordering, engine mode, and binding options, see [Policy bindings and evaluation](./bindings.mdx).
